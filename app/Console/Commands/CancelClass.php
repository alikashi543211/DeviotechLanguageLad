    ...
    public function handle()
    {
        try
        {
            ...
            if($booking_list->count() > 0)
            {
                ...
                {
                    $now_date = Carbon::now();
                    $req_date = Carbon::parse($booking->created_at);
                    ...
                    if($result)
                    {
                       $hours = $req_date->diffInHours($now_date);
                        if ($hours >= 12) 
                        {
                            ...
                            booking by the teacher has expired. The class has been returned to your profile, so 
                    ...
                    if($result)
                    {
                       $hours = $req_date->diffInHours($now_date);
                        if ($hours >= 12) 
                        {
                            ...
                            $booking->cancel_by = "LanguageLad";
                            $booking->cancel_reason = "The time limit for accepting your 
                            trial request by the teacher has expired. We apologise for the inconvenience.";
                            $booking->save();
        ...
        Log::info("Bookings And Trials Cancelled Successfully");
        
        }catch(\Exception $e)
        {
            Log::info("Cancel Class Crone Job Error : ".$e->getMessage());
        }
    }
}