                        ...
                        <div class="tabs-box profile-tabs">
                            <ul class="tab-btns tab-buttons clearfix">    
                                <li data-tab="#pending_requests" class="tab-btn active-btn">
                                    <a data-toggle="tab" href="#pending_requests">
                                        Requests
                ...
                <div class="modal-content dashboard-section p-0">
                    <div class="styled-form">
                        <form action="" method="GET" class="custom-offer-form crm_form">
                            <div class="modal-header">
                                <h5 class="modal-title main_title" id="exampleModalLongTitle">Booking Cancel Request</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content dashboard-section p-0">
                <form action="" method="POST" class="custom-offer-form">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title main_title" id="exampleModalLongTitle">Complete Booking</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body" id="seller-location-body">
                      Are you sure want to complete the booking?
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
@section('js')
    <script>
        ...
        load_cancelled_requests();
        show_teacher_time_change_modal();


        function load_pending_requests($url)
        {
            $.ajax({
                type: "GET",
                url: "{{ route('student.booking_request.load.list') }}",
        ...
        function load_active_requests($url)
        {
            $.ajax({
                type: "GET",
                url: "{{ route('student.booking_request.load.active') }}",
                success: function (response) {
                    $('#active_requests').html(response);
                    start_data_table();
                }
            });
        }

        function load_completed_requests($url)
        {
            $.ajax({
                type: "GET",
                url: "{{ route('student.booking_request.load.completed') }}",
                success: function (response) {
                    $('#completed_requests').html(response);
                    start_data_table();
                }
            });
        }

        function load_cancelled_requests($url)
        {
            $.ajax({
                type: "GET",
                url: "{{ route('student.booking_request.load.cancelled') }}",
                success: function (response) {
                    $('#cancelled_requests').html(response);
                    start_data_table();
                }
            });
        }
