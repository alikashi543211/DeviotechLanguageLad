                            ...
                            <tbody>
                                @foreach($booking_list as $item)
                                    <tr>
                                        ...
                                        {{-- <td>
                                            @if(is_stripe_payment($item->id) && $item->refund_status == '0')
                                                <a href="javascript:void(0);" title="Send Refund Request" data-title="Are you sure want to send the refund request?" data-href="{{ route('student.booking_request.refund.request', $item->id) }}" class="btn btn-warning mr-1" id="check-in-location-view"><i class="fa fa-paper-plane"></i></a>
                                            @endif
